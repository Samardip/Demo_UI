...
import { JobType, setInterestState, UserProfileData } from "../../ReduxStateManagement/actions/Action";
import axios from "axios";
export const InterestedDialogBox = ({ onClose }) => {
  const [data, setdata] =useState({});
  let obj8=[];
  const Istate = useSelector(state => state.UpdateDatas.IState)
  const Ioption=useSelector((state)=>state.UpdateDatas.IOptions);
  ...
  const obj4 =
    obj6 === undefined
      ? []
      : obj6.map((d) => {
          return { title: d.interested_work_in_name,
                   id:d.interested_work_in_id, };
        });
  console.log(obj4);
  joboptions=obj4;
  ...
  const defalutobj=Istate.map((is,i)=>{
...
console.log(defalutobj);
  const SaveUpdatedDesc = () => {
    if(Istate.length!==0 && obj8.length===0)
      obj8=defalutobj;
    console.log(obj8);
    ...
    console.log(obj31);
    dispatch(setInterestState(obj31));
    const interestedpost = {
      interested_work_in_id: obj3,
      actionby_id: 1097,
      user_id: 1097,
    };
    axios
      .post("https://develop.hipoz.com/api/updatestudentinterestedworkin", interestedpost)
      .then((response) => {
...
let arrayObj=[];
 for(var i=0;i<Istate.length;i++)
 {
   ...
   {
     if(Istate[i]===joboptions[j].title)
     {
        arrayObj=[...arrayObj,j];
     }
   }
